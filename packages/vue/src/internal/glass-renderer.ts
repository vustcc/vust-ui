...
/** A rounded-rectangle normal field: neutral center, refracting curved rim. */
    ...
    for (let x = 0; x < w; x++) {
      ...
      const ny = length
        ...
        : qy >= qx
          ...
          : 0;
      const i = (y * w + x) * 4;
      pixels.data[i] = Math.round(127.5 + nx * bend * 127.5);
      pixels.data[i + 1] = Math.round(127.5 + ny * bend * 127.5);
      pixels.data[i + 2] = 128;
      pixels.data[i + 3] = 255;
    }
  }
...
export function mountGlass(element: HTMLElement, profile: VustGlassProfile) {
  ...
  // This is a conservative renderer selection, not a pixel-support assertion.
  ...
  function paint() {
    ...
    if (mode === "refraction") {
      ...
      const radius = radiusText.endsWith("%")
        ? (Math.min(width, height) * parseFloat(radiusText)) / 100
        : parseFloat(radiusText) || 0;
